...
const createAuditMessage = (action, context) => {
  const messages = {
    USER_LOGIN: `User ${context.userName} (${
      context.userEmail
    }) successfully logged in from ${
      context.branchLocation || "Unknown Branch"
    } at ${new Date().toLocaleTimeString()}`,
    USER_SETUP_COMPLETED: `New user ${context.userName} (${
      context.userEmail
    }) completed account setup and was assigned role '${
      context.roleName
    }' for ${context.branchLocation || "Unknown Branch"}`,
    PRODUCT_REQUEST_CREATED: `Product requisition request #${
      context.requestId
    } was created by ${context.userName} (${context.userEmail}) from ${
      context.branchLocation || "Unknown Branch"
    } requesting ${context.itemCount} items with total quantity of ${
      context.totalQuantity
    } units`,
    PRODUCT_REQUEST_APPROVED: `Product requisition request #${
      context.requestId
    } was approved by ${context.reviewerName} (${context.reviewerRole}) from ${
      context.reviewerBranch || "Unknown Branch"
    }. Request contained ${context.itemCount} items with total quantity of ${
      context.totalQuantity
    } units`,
    PRODUCT_REQUEST_DENIED: `Product requisition request #${
      context.requestId
    } was denied by ${context.reviewerName} (${context.reviewerRole}) from ${
      context.reviewerBranch || "Unknown Branch"
    }. Request contained ${context.itemCount} items with total quantity of ${
      context.totalQuantity
    } units`,
    PRODUCT_ADDED: `New product '${context.productName}' was added to ${
      context.branchLocation || "Unknown Branch"
    } with initial quantity of ${context.quantity} units at $${
      context.price
    } per unit`,
    PRODUCT_UPDATED: `Product '${context.productName}' (ID: ${
      context.productId
    }) was updated in ${context.branchLocation || "Unknown Branch"}. Changes: ${
      context.changes
    }`,
    PRODUCT_DELETED: `Product '${context.productName}' (ID: ${
      context.productId
    }) was deleted from ${context.branchLocation || "Unknown Branch"} by ${
      context.userName
    }`,
    USER_CREATED: `New user ${context.userName} (${
      context.userEmail
    }) was created with role '${context.roleName}' for ${
      context.branchLocation || "Unknown Branch"
    }`,
    ...
    BRANCH_CREATED: `New branch '${context.branchName}' was created at ${
      context.branchAddress || "Unknown Address"
    }`,
  ...
  return (
    messages[action] ||
    `Action '${action}' was performed by ${context.userName || "Unknown User"}`
  );
};
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `Bulk importing ${products.length} products by user: ${user.name} (${user.email})`
      );
      ...
      if (zeroQuantityProducts.length > 0) {
        console.log(
          `Found ${zeroQuantityProducts.length} products with zero quantity:`,
        ...
        for (const product of batch) {
          try {
            ...
            if (checkError && checkError.code !== "PGRST116") {
              results.errors.push(
                `Error checking product "${product.name}": ${checkError.message}`
              );
              continue;
            }
            ...
            if (existingProduct) {
              ...
              if (updateError) {
                results.errors.push(
                  `Error updating product "${product.name}": ${updateError.message}`
                );
              } else {
                results.updated++;
              ...
              if (createError) {
                results.errors.push(
                  `Error creating product "${product.name}": ${createError.message}`
                );
              } else {
                results.created++;
...
app.delete("/api/products/:id", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    if (requestCheckError) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to check product dependencies" });
    }
    ...
    if (activeRequests && activeRequests.length > 0) {
      console.log(
        `Deleting ${activeRequests.length} completed/denied request items for product ${id}`
      );
      ...
      if (deleteItemsError) {
        console.error(
          "Error deleting related request items:",
          deleteItemsError
        );
        return res
          .status(500)
          .json({ error: "Failed to clean up related request items" });
      }
    }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!data.role || !data.role.role_name)
        return res
          .status(500)
          .json({ error: "User role information is missing" });
...
app.put("/api/branches/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { location, address, latitude, longitude, map_snapshot_url } =
      req.body;

    if (!location || !address) {
      return res
        .status(400)
        .json({ error: "location and address are required" });
    }
...
app.post(
  ...
  rateLimits.productRequests,
  validateRequest(schemas.productRequest),
  async (req, res) => {
    try {
      ...
      for (const item of items) {
        ...
        const availableQuantity =
          productData.quantity - (productData.reserved_quantity || 0);
...
app.get(
  "/api/product-requests/pending/:userId",
  rateLimits.productRequests,
  async (req, res) => {
    try {
      ...
      const transformedData = data.map((request) => ({
        ...request,
        requester_branch:
          request.requester?.branch?.location || "Unknown Branch",
        items: request.items.map((item) => ({
          ...
          category_name:
            item.product?.category?.category_name || "Unknown Category",
        })),
      }));
...
app.get(
  "/api/product-requests/sent/:userId",
  rateLimits.productRequests,
  async (req, res) => {
    try {
      const { userId } = req.params;
      ...
      const transformedData = data.map((request) => ({
        ...request,
        recipient_branch:
          request.recipient?.branch?.location || "Unknown Branch",
...
app.put(
  "/api/product-requests/:requestId/review",
  rateLimits.productRequests,
  async (req, res) => {
    try {
      ...
      if (!action || !reviewedBy || !["approved", "denied"].includes(action)) {
        return res
          .status(400)
          .json({ error: "Invalid action or missing required fields" });
      }
      ...
      if (action === "approved") {
        ...
        if (requesterError) {
          ...
          return res
            .status(500)
            .json({ error: "Failed to fetch requester information" });
        }
        ...
        for (const item of requestData.items) {
          ...
          const { data: productData, error: fetchError } = await supabase
            .from("centralized_product")
            .select(
              "quantity, reserved_quantity, product_name, price, category_id, status"
            )
          ...
          const newQuantity = Math.max(
            0,
            productData.quantity - reservedAmount
          );
          const newReservedQuantity = Math.max(
            0,
            (productData.reserved_quantity || 0) - reservedAmount
          );
          ...
          if (updateError) {
            console.error(
              "Error updating source product quantity:",
              updateError
            );
            continue;
          }
      ...
      const { data: reviewerData } = await supabase
        .from("user")
        .select("name, role:role_id(role_name), branch:branch_id(location)")
      ...
      const auditContext = {
        ...
        totalQuantity: requestData.items.reduce(
      ...
      const { error: auditError } = await supabase.from("audit_logs").insert([
        {
          ...
          metadata: {
            ...
            total_quantity: requestData.items.reduce(
              (sum, item) => sum + item.quantity,
              0
            ),
...
app.get(
  "/api/product-requests/pending-for-branch/:branchId",
  async (req, res) => {
    try {
      const { branchId } = req.params;
      ...
      if (!branchManagerData) {
        return res
          .status(404)
          .json({ error: "No Branch Manager found for this branch" });
      }
    ...
    const transformedData = await Promise.all(
      (data || []).map(async (log) => {
        const enrichedLog = {
          ...
          action_category: log.action?.includes("LOGIN")
            ? "Authentication"
          ...
          severity_level:
            log.action?.includes("DELETE") || log.action?.includes("REMOVE")
              ...
              ? "Low"
              : log.action?.includes("LOGIN") || log.action?.includes("VIEW")
              ? "Info"
              : "Medium",
          time_period:
        ...
        if (log.entity_type === "product_requisition" && log.entity_id) {
          ...
          if (!requisitionError && requisitionData) {
            ...
            enrichedLog.requester_branch =
              requisitionData.requester_branch_location;
...
app.get("/api/audit-logs/:id", async (req, res) => {
  try {
    ...
    // Fetch the specific audit log from the view
    const { data, error } = await supabase
      .from("v_audit_logs_detailed")
      .select("*")
      .eq("id", id)
...
// GET related activities for a specific entity
app.get("/api/audit-logs/related/:entityType/:entityId", async (req, res) => {
  try {
    const { entityType, entityId } = req.params;

    // Fetch related audit logs for the same entity
    const { data, error } = await supabase
      .from("v_audit_logs_detailed")
      .select("*")
    ...
    res.status(500).json({ error: "Failed to fetch related activities" });
  }
});
...
app.get("/api/dashboard/stats", rateLimits.dashboardStats, async (req, res) => {
  try {
    ...
    let stockQuery = supabase.from("centralized_product").select("quantity");
    ...
    const { count: outOfStockCount, error: outOfStockError } =
      await outOfStockQuery;
...
app.get("/api/audit-logs/stats", rateLimits.auditLogs, async (req, res) => {
  try {
    ...
    data.forEach((log) => {
      ...
      stats.actions_by_type[log.action] =
        (stats.actions_by_type[log.action] || 0) + 1;

      // Count by user
      stats.actions_by_user[log.user_id] =
        (stats.actions_by_user[log.user_id] || 0) + 1;
    });
...
app.post(
  "/api/create_pending_user",
  rateLimits.userCreation,
  validateRequest(schemas.pendingUser),
  async (req, res) => {
    const user = req.body;
    ...
    try {
      ...
      if (
        (checkError && checkError.code !== "PGRST116") ||
        (checkPendingError && checkPendingError.code !== "PGRST116")
      ) {
        console.error(
          "Error checking existing user:",
          checkError || checkPendingError
        );
        return res.status(500).json({
          error: "Error checking for existing user",
      ...
      res.status(201).json({
        ...
        message:
          "Pending user created successfully. Setup link generated and email sent.",
      });
    } catch (error) {
