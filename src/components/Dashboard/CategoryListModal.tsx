...
const CategoryListModal = ({ isOpen, onClose }: CategoryListModalProps) => {
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      const { error, data } = await api.deleteCategory(
        deletingCategory.id,
        currentUser.user_id
      );
      ...
      if (error) {
        ...
        if (error.includes("being used by") || error.includes("product(s)")) {
          toast.error(error, {
            duration: 6000,
            style: {
              background: "#dc2626",
      ...
      toast.success(`Category "${deletingCategory.category_name}" deleted successfully!`, {
        duration: 3000,
      });
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black/50 dark:bg-black/70 z-[60]">
            <div className="bg-white dark:bg-gray-900/90 rounded-lg shadow-lg p-6 w-full max-w-md mx-4">
              ...
              <p className="text-sm text-yellow-600 dark:text-yellow-400 mb-4">
                Note: Categories that are being used by products cannot be deleted.
              </p>
              <div className="flex justify-end gap-3">
                <button
                  onClick={() => setDeletingCategory(null)}
