...
class ApiClient {
    ...
    // GET request
    async get<T>(endpoint: string, params?: Record<string, any>): Promise<{ data: T | null; error: string | null }> {
...
export const api = {
    ...
    createUser: (userData: any) =>
        apiClient.post('/create_users', userData),
    ...
    getUserByToken: (token: string) =>
        apiClient.get(`/user_by_token/${token}`),
    ...
    reviewRequest: (requestId: number, action: string, reviewedBy: string, notes?: string) =>
        apiClient.put(`/product-requests/${requestId}/review`, { action, reviewedBy, notes }),

    getPendingRequestsForBranch: (branchId: number) =>
        apiClient.get(`/product-requests/pending-for-branch/${branchId}`),

    getAllRequests: () =>
        apiClient.get('/product-requests/all'),
